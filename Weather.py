...
ph0 = ["На улице дождь, возьми зонтик.","Синоптики обещают дождь, но это не точно."]
...
ph4 = ["На улице тепло, надевай что хочешь.","На улице тепло, можно прогуляться.", "На улице отличная погодка для прогулки."]
ph5 = ["На улице адская жара.","На улице жара, возьми с собой водички.","На улице пахнет жаренным... Кто-то решил прогуляться."]
ph6 = ["Там то все тает, то снова замерзает, надевай шипованные ботинки.", "Запрягай коньки, на улице скорее всего будет каток."]
ph7 = ["На улице холодно, лучше надеть перчатки.", "На улице температура не райская, но и ты не на Богамах.", "На улице холодно, рекомендую надеть подштанники."]
...
def send_echo(message):
	...
	try:
		...
		if message.text.lower() in uncens:
